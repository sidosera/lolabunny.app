...
func run(arguments: [String]) throws -> Int32 {
    ...
    case "bindings", "list":
        printCommands(CommandRouter().allCommands())
    case "completion":
        print("# Shell completion generation is not required for the Swift backend.")
    case "pid-file":
        print(Paths.pidFile.path)
    case "blob":
        try handleBlob(Array(parsed.positionals.dropFirst()), config: config)
    default:
    ...
    while index < arguments.count {
        ...
        case "--history-max-entries":
            ...
            guard let count = Int(raw), count > 0 else {
                throw BackendError.message("invalid --history-max-entries: \(raw)")
            }
    ...
    while index < arguments.count {
        ...
        case "-p", "--port":
            ...
            guard let port = UInt16(raw) else {
                throw BackendError.message("invalid --port: \(raw)")
            }
...
func value(after flag: String, in arguments: [String], index: inout Int) throws -> String {
    ...
    guard valueIndex < arguments.count else {
        throw BackendError.message("missing value for \(flag)")
    }
    index += 2
...
func parseAlias(_ raw: String) throws -> (key: String, value: String) {
    ...
    guard parts.count == 2 else {
        throw BackendError.message("alias must be KEY=VALUE, got '\(raw)'")
    }
...
func parseBool(_ raw: String) throws -> Bool {
    ...
    case "0", "false", "no", "off":
        return false
    default:
        throw BackendError.message("invalid boolean: \(raw)")
    }
}
...
func openURL(_ url: String, browser: String?) throws {
    ...
    guard process.terminationStatus == 0 else {
        throw BackendError.message("failed to open URL")
    }
}

func handleBlob(_ arguments: [String], config: AppConfig) throws {
    if let id = arguments.first {
        let url = URL(string: "\(config.server.displayURL)/blob/\(id)/raw")!
    ...
    guard !input.isEmpty else {
        throw BackendError.message("Usage: echo 'content' | lolabunny blob")
    }

    let url = URL(string: "\(config.server.displayURL)/blob")!
    let response = try syncHTTPPost(url, body: input)
    guard let text = String(data: response, encoding: .utf8) else {
        throw BackendError.message("unexpected server response")
    }
    let parts = text.split(separator: "\t", omittingEmptySubsequences: false)
    guard parts.count == 3 else {
        throw BackendError.message("unexpected server response: \(text)")
    }
    print(parts[0])
}

func syncHTTPPost(_ url: URL, body: Data) throws -> Data {
    ...
    URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        if let http = response as? HTTPURLResponse,
           !(200..<300).contains(http.statusCode) {
            let message = String(data: data ?? Data(), encoding: .utf8) ?? "server returned \(http.statusCode)"
            result.set(.failure(BackendError.message(message)))
            return
        }
