...
extension AppDelegate: UNUserNotificationCenterDelegate {
    private var canUseUserNotificationCenter: Bool {
        Bundle.main.bundleIdentifier != nil && Bundle.main.bundleURL.pathExtension == "widget"
    }

    func configureNotificationActions() {
        guard canUseUserNotificationCenter else {
            log("skipping notification setup – unavailable main widget bundle")
            return
        }
    ...
    func postServerUpdateReadyNotification(_ version: String) {
        postNotification(
    ...
    func postNotification(
        ...
        guard canUseUserNotificationCenter else {
            log("skipping notification post – unavailable main widget bundle: \(title) – \(body)")
            return
        }
    ...
    public nonisolated func userNotificationCenter(
        ...
        if content.categoryIdentifier == Config.Notification.updatePromptCategory {
            ...
            guard let version = content.userInfo[Config.Notification.serverVersionKey] as? String else {
                log("missing widget-server version in update notification payload")
                return
            }
        ...
        if content.categoryIdentifier == Config.Notification.bootstrapPromptCategory {
            ...
            Task { @MainActor [weak self] in
                await self?.beginBootstrapServerDownload(requiredMajor: requiredMajor)
            }
        }
    }
